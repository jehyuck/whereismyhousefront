                ...
                <select name="dong" id="dong" v-model="dong">
                  <option v-for="(dongitem, index) in dongList" :key="index" :value="dongitem.value">{{ dongitem.name }}</option>
								...
								<div class="col-lg-3 col-md-3 col-12 p-0 search-btn button">
									<button @click="addBookmark" class="btn" type="submit" form="searchForm">
										<i class="lni lni-search-alt"></i> Add
									</button>
								</div>
          </div>
        </div>
...
export default {
  ...
  methods: {
    ...
    addBookmark() {
      ...
      http.post(`bookmark`, {"dong":this.dong, "userid":this.userInfo.id})
      .then(({ data }) => {
        if (data == 'success') {
            alert('북마크 추가 완료');
          }
